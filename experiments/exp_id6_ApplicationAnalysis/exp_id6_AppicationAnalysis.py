...
def execute(parameter):
    ...
    node_info = {f'Node {i}': {'coordinate': (int(i*50), 0, 0)} for i in range(num_nodes)}
    edge_info = {
        (f'Node {i}', f'Node {i+1}'): {
    ...
    exps = Xperiment(
        ...
        experiment = f'exp_id6_AppicationAnalysis_p{index}_r{repeat_th}'
    )
...
num_trajectories = 10
exp_names = ['0G', '1G-Ss-Dp', '2G-NonLocalCNOT', '1-2G-DirectedEncoded', 'HG-END2ENDPurifiedEncoded']
...
# %%
message_log = 'exp_id6_AppicationAnalysis'
                    ...
                    for measure_error in measurement_error_list:
                        for exp_name in exp_names:
                            for trajectory in range(num_trajectories):
                                ...
                                endNode = f'Node {hops}'
                                QubitWaitingTime = {'Node 0': [], endNode: []}

                                for BellPair in exp['Qubits waiting time']:
                                    QubitWaitingTime['Node 0'].append(BellPair['Node 0'])
                                    QubitWaitingTime[endNode].append(BellPair[endNode])
                                ...
                                data = {
                                    ...
                                    #'qubits waiting time (Left node)': QubitWaitingTime['Node 0'],
                                    #'qubits waiting time (Right node)': QubitWaitingTime[f'Node {hops}']
                                }
                                parameters_set.append(data)
                        index += 1

DataFrame = pd.DataFrame(parameters_set)
DataFrame.to_csv('exp_id6_AppicationAnalysis_Extracted_Data.csv', index=False)
